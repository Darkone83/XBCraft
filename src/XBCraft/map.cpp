/*---------------------------------------------------------------------------
    ...
    No changes to logic from original Craft -- pure C, no external deps.
    C89 cleanup: variable declarations hoisted above statements throughout.
---------------------------------------------------------------------------*/
...
{
    unsigned int index = (unsigned int)hash(x, y, z) & map->mask;
    MapEntry* entry;
    int overwrite = 0;
