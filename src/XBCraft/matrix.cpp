/*---------------------------------------------------------------------------
    ...
    C89 cleanup from original Craft:
      - mat_rotate:      float s/c/m declared before normalize() call
      - mat_vec_multiply: for-loop var declarations hoisted
      - mat_multiply:    for-loop var declarations hoisted; loop var
                         renamed from 'c' to 'col' for clarity
      - mat_apply:       for-loop var declaration hoisted
    All double literals replaced with float literals (2.0f, 0.0f etc.)
    Logic is identical to the original throughout.
---------------------------------------------------------------------------*/
...
void mat_rotate(float* matrix, float x, float y, float z, float angle)
{
    ...
    c = cosf(angle);
    m = 1.0f - c;
    matrix[0] = m * x * x + c;   matrix[1] = m * x * y - z * s; matrix[2] = m * z * x + y * s; matrix[3] = 0;
    matrix[4] = m * x * y + z * s; matrix[5] = m * y * y + c;   matrix[6] = m * y * z - x * s; matrix[7] = 0;
    matrix[8] = m * z * x - y * s; matrix[9] = m * y * z + x * s; matrix[10] = m * z * z + c;   matrix[11] = 0;
    matrix[12] = 0;            matrix[13] = 0;            matrix[14] = 0;            matrix[15] = 1;
}
...
void mat_apply(float* data, float* matrix, int count, int offset, int stride)
{
    ...
    vec[0] = 0.0f; vec[1] = 0.0f; vec[2] = 0.0f; vec[3] = 1.0f;
...
void frustum_planes(float planes[6][4], int radius, float* matrix)
{
    ...
    planes[4][0] = znear * m[3] + m[2];  planes[4][1] = znear * m[7] + m[6];
    planes[4][2] = znear * m[11] + m[10]; planes[4][3] = znear * m[15] + m[14];

    planes[5][0] = zfar * m[3] - m[2];  planes[5][1] = zfar * m[7] - m[6];
    planes[5][2] = zfar * m[11] - m[10]; planes[5][3] = zfar * m[15] - m[14];
}
...
{
    mat_ortho(matrix, 0, (float)width, 0, (float)height, -1.0f, 1.0f);
}
    ...
    float fov, int ortho, int radius)
{
    ...
    if (ortho)
    {
        int size = ortho;
        mat_ortho(b,
            -(float)size * aspect, (float)size * aspect,
...
void set_matrix_item(float* matrix, int width, int height, int scale)
{
    float a[16], b[16];
    float aspect = (float)width / (float)height;
    float size = 64.0f * (float)scale;
    float box = (float)height / size / 2.0f;
    float xoffset = 1.0f - size / (float)width * 2.0f;
    float yoffset = 1.0f - size / (float)height * 2.0f;

    mat_identity(a);
    mat_rotate(b, 0, 1, 0, -PI / 4.0f);
    mat_multiply(a, b, a);
    mat_rotate(b, 1, 0, 0, -PI / 10.0f);
    mat_multiply(a, b, a);
    mat_ortho(b, -box * aspect, box * aspect, -box, box, -1.0f, 1.0f);
    mat_multiply(a, b, a);
