/*---------------------------------------------------------------------------
    ...
    C89 cleanup from original Craft:
      - ring_put: RingEntry *e hoisted before the ring_full check
      - ring_get: RingEntry *e hoisted before the ring_empty check
...
{
    ring->capacity = (unsigned int)capacity;
    ring->start = 0;
    ring->end = 0;
...
int ring_size(Ring* ring)
{
    if (ring->end >= ring->start)
        return (int)(ring->end - ring->start);
    else
        return (int)(ring->capacity - (ring->start - ring->end));
}
...
void ring_grow(Ring* ring)
{
    Ring new_ring;
    RingEntry entry;
    ring_alloc(&new_ring, (int)(ring->capacity * 2));
